        ...
        try {
            if (typeof condValueOrg == "string" && condValueOrg.indexOf("{") > -1 && condValueOrg.indexOf("}") > -1) { //contains other field reference
                condValueOrg = condValueOrg.replace(/\{.*?\}/gi, function (match, contents, offset, s) {
                    ...
                    var elements = $$('input[name$="_' + stripped + '"], input[name$="_' + stripped + '[date]"]');
                    ...
                    if (spans.length == 0) {
                      if (resultFieldType === 'inline') {
                        ...
                        spans.each(function (span) {
                            // BUGFIX #2934064 - If there is no context in `output` -> .update() returns an empty innerText, I've added a precheck to prevent that. We should be able to render plain text.
                            if (output) {
   ...
   */
    ...
    highLightLines: function () {
        ...
        $$('.form-line').each(function (l, i) {
            l.select('input, select, textarea, div, table div, button, div div span[tabIndex]:not([tabIndex="-1"]), a, iframe, label[tabIndex]').each(function (i) {
                // We can't listen focus event input when in iframe. If iframe in collapsed menu, focus label before input for open collapsed menu.
                if(i.tagName === 'IFRAME' && JotForm.isCollapsed(l)) {
                    l.querySelector("label").setAttribute('tabindex', 0);
                }
        ...
        JotForm.createXHRRequest(url, 'get', null, function(res) {
            $H(res.data).each(function(pair) {
                ...
                if (questionType === 'control_datetime') {
                    if (pair.key.indexOf('hour_') > -1 && inputValue.charAt(0) === '0') {
